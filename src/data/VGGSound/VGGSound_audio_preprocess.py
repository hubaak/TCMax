...
dataset_path = Dataset_Config().get_dataset_root("VGGSound")

dataset_config = {
    'audio_path' : 'audio_train',
    'audio_path2' : 'audio_test',
...
if __name__ == "__main__":
    ...
    preprocessed_and_save(audio_path, max_workers=64)
    
    audio_dataset_p = os.path.join(dataset_path, dataset_config['audio_path2'])
    audio_out_dataset_p = os.path.join(dataset_path, dataset_config['out_path'])
    os.makedirs(audio_out_dataset_p, exist_ok=True)
    audio_path = get_path_from_dir(audio_dataset_p, audio_out_dataset_p)
    print(len(audio_path))
    preprocessed_and_save(audio_path, max_workers=4)
